...
import { ReservationData } from "../books/utils";
...
type UserSection = "profile" | "password" | "reservations";
...
const UserPage = () => {
  ...
  // Reservations state
  const [reservationsPage, setReservationsPage] =
    useState<PageResponse<ReservationData> | null>(null);
  const [reservationsPageInfo, setReservationsPageInfo] =
  ...
  const [reservationsPageIndex, setReservationsPageIndex] =
    useState(0); // 0-based
  ...
  useEffect(() => {
    ...
    const fetchReservations = async (pageIndex: number) => {
      setReservationsLoading(true);
      setReservationsError(null);

      try {
        const res = await axiosClient.get<
          PageResponse<ReservationData>
        ...
        setReservationsPageInfo(res.data.page);
      } catch (err: any) {
        console.error(err);
        const messages = getErrorMessagesFromError(err, {
          defaultMessage: "Unable to load your reservations.",
        });
        setReservationsError(messages.join(" "));
      } finally {
        setReservationsLoading(false);
      }
    };

    void fetchReservations(reservationsPageIndex);
  }, [activeSection, reservationsPageIndex]);
  ...
  const canResPrev = reservationsPageInfo
    ? reservationsPageInfo.number > 0
    : false;
  const canResNext = reservationsPageInfo
    ? reservationsPageInfo.number + 1 <
      reservationsPageInfo.totalPages
    : false;
              ...
              <div className="rounded-2xl bg-white shadow-md border border-amber-100 p-5 sm:p-6">
                ...
                >
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-2">
                    <p className="text-xs text-slate-600">
                      Changes are applied to your Digilib account and
              ...
              <div className="rounded-2xl bg-white shadow-md border border-amber-100 p-5 sm:p-6">
                ...
                {!reservationsLoading &&
                  ...
                  reservationsPage.content.length === 0 && (
                    <div className="py-8 text-sm text-slate-600 text-center">
                  ...
                  reservationsPage.content.length > 0 && (
                    <>
                      <div className="overflow-x-auto">
                        <table className="min-w-full text-sm md:text-base border-t border-b border-slate-200">
                          <thead>
                      ...
                      {reservationsPageInfo && (
                        <div className="flex items-center justify-between mt-4 text-xs text-slate-600">
                          <div>
                            Page {reservationsPageInfo.number + 1} of{" "}
                            {reservationsPageInfo.totalPages} ·{" "}
                            {reservationsPageInfo.totalElements}{" "}
                            reservations
                          </div>
                          <div className="flex gap-2">
                            <button
                              ...
                              onClick={() =>
                                canResPrev &&
                                setReservationsPageIndex((p) => p - 1)
                              }
                              className="rounded-full border border-slate-200 px-3 py-1 disabled:opacity-50 disabled:cursor-not-allowed text-slate-800 hover:bg-slate-50 transition"
                            >
                              Previous
                            </button>
                            <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl border border-amber-100 p-5 sm:p-6">
            ...
            <p className="text-sm text-slate-600 mb-4">
              You&apos;re about to update your account details. Please
